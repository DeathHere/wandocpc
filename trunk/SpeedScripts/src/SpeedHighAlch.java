/**
 * Educational code to simulate a human performing the high alch spell
...
public class SpeedHighAlch extends Script implements ServerMessageListener, PaintListener {
    ...
    private boolean alchemy() {
        ...
        while (!isPaused && checks()) {
            ...
            if (itemPos.distance(getMouseLocation()) < 30 && getMenuIndex("Cast High") != -1) {
            } else {
            ...
            if (getMenuIndex("Cast High") == getMenuIndex(">") && getMenuIndex("Cast High") != -1) {
                clickMouse(true);
            }
        }
        return false;
