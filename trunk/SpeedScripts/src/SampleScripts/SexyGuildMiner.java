...
@ScriptManifest(name = "Sexy Miner", authors = { "Imasexybeast", "Fusion89k",
		"Asamin", "Fred" }, category = "Mining", version = 1.41, description = "<html><body><b>All settings are on the GUI =)</b><br><img src=\"http://i126.photobucket.com/albums/p98/ketekcomp/sgm.png\" /></body></html>")
public class SexyGuildMiner extends Script implements PaintListener,
	...
	int[] pickaxeID = { 1265, 1267, 1269, 1273, 1271, 1275, 15261 };
	...
	public int getNewPick() {
		...
		if (distanceTo(bankTile) > 4) {
			if (distanceTo(randTile) > 5 && randTile.getX() != nullTile.getX()
					&& getMyPlayer().isMoving()) {
				return getNewPick();
			}
			randTile = new RSTile(bankTile.getX() + random(-1, 2), bankTile
					.getY()
					+ random(-1, 2));
			try {
				SGMWalk(randTile);
		...
		if (!RSInterface.getInterface(Constants.INTERFACE_BANK).isValid()) {
			...
			if (r == 0) {
				if (!atObject(getNearestObjectById(15, bankBoothID),
						"Use-Quickly")) {
					if (r == 1) {
						if (!atNPC(getNearestNPCByID(bankerID), "Bank")) {
							wait(random(100, 100));
						}
					}
				}
		...
		if (bank.isOpen()) {
			...
			if (getMenuItems().get(0).toLowerCase().contains(
					"pickaxe".toLowerCase())
					&& !getMenuItems().get(0).toLowerCase().contains(
							"Broken".toLowerCase())) {
				clickMouse(true);
				wait(random(10, 10));
			} else {
	...
	public void mouseOverMiningSkill() {
		...
		final RSInterfaceChild one = RSInterface.getInterface(320).getChild(9);
		final RSInterfaceChild two = RSInterface.getInterface(320).getChild(10);
		if (random(0, 2) == 0) {
			moveMouse(new Point(one.getAbsoluteX()
					+ random(2, one.getWidth() - 1), one.getAbsoluteY()
					+ random(2, one.getHeight() - 1)));
	...
	public int banking() {
		if (!RSInterface.getInterface(Constants.INTERFACE_BANK).isValid()) {
			...
			if (r == 0) {
				if (getNearestNPCByID(bankerID) == null) {
					return random(200, 300);
				}
				while (!Calculations
						.onScreen(Calculations.tileToScreen(getNearestNPCByID(
								bankerID).getLocation()))) {
					wait(10);
				}
				atNPC(getNearestNPCByID(bankerID), "ank Ba");
			} else {
				if (getNearestObjectById(15, bankBoothID) == null) {
					return random(200, 300);
				}
				while (!Calculations.onScreen(Calculations
						.tileToScreen(getNearestObjectById(15, bankBoothID)
								.getLocation()))) {
					wait(10);
				}
				atObject(getNearestObjectById(15, bankBoothID), "uickl");
			}
			wait(random(200, 300));
