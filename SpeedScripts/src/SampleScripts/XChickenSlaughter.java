...
@ScriptManifest(authors = { "BeanMan Xscripting Inc." }, category = "Combat", name = "XChickenSlaughter", version = 4.01, description = "<html><head>"
		+ "</head><body>"
		+ "<center><img src=\"http://binaryx.nl/xscripting/beanman/XChickenSlaughter/xchickenslaughter.png\" /></center>"
		+ "<center><strong>All options are in the GUI</strong></center>"
		+ "<center></center>"
		+ "<center>Start the script at lumbridge, falador or guild</center>"
		+ "<center></center>"
		+ "<strong>Note: </strong>"
		+ "The bot might have some problems walking to the location so I suggest starting the script immediately at the right spot. "
		+ "<br></br>"
		+ "<strong>Note2: </strong>"
		+ "This script checks for script-updates and prices of items online at start"
		+ "<center><strong>Script made by BeanMan - XScripting Inc.</strong></center>"
		+ "</body></html>")
public class XChickenSlaughter extends Script implements PaintListener,
	...
	public class GarrettsPaint {
		...
		public void paint(final Graphics g) {
			...
			if (advancedPaint) {
				...
				if (itemPresent(bonesID) && (takeBones1 || takeBones2)
						&& getNearestFreeNPCToAttackByName("Chicken") == null) {
	...
	public class KillDaChicksGUI extends JFrame {
		...
		private void initComponents() {
			...
			// ---- label22 ----
			label22
					.setText("Food is not needed, since even a level 3 cannot die by a chicken!");
			label22.setIcon(UIManager.getIcon("OptionPane.warningIcon"));
			contentPane.add(label22);
			label22.setBounds(new Rectangle(new Point(25, 550), label22
					.getPreferredSize()));

			// ---- label27 ----
			label27
					.setText("This script also drops everything except for feathers, arrows, bones and coins!");
			label27.setFont(label27.getFont().deriveFont(
					label27.getFont().getStyle() & ~Font.BOLD));
			contentPane.add(label27);
			label27.setBounds(new Rectangle(new Point(25, 585), label27
					.getPreferredSize()));
	...
	private enum State {
		FIGHTING, ATTACK, PICKUPFEATHERS, PICKUPBONES, PICKBRONZEARROW, PICKIRONARROW, PICKSTEELARROW, PICKMITHRILARROW, PICKADDYARROW, PICKRUNEARROW, BURY, DROP, WAIT, SETRUN, WELCOMESCREEN, TOLOCATION, HOVERTONEXTCHICKEN, DROPBONES, EQUIPBRONZE, EQUIPIRON, EQUIPSTEEL, EQUIPMITHRIL, EQUIPADDY, EQUIPRUNE, STOPSCRIPT
	}
	...
	public int thingsToDrop[] = { 2138, 1944, 1351 };
	public int bronzeArrowID = 882, ironArrowID = 884, steelArrowID = 886,
			mithrilArrowID = 888, addyArrowID = 890, runeArrowID = 892;
	...
	public int feathersID = 314, bonesID = 526, featherMarketPrice,
			...
			stopAtLevel, Xmin, Xmax, Ymin, Ymax;

	public int thingsnottodrop[] = { bronzeArrowID, ironArrowID, steelArrowID,
			mithrilArrowID, addyArrowID, runeArrowID, feathersID, bonesID,
			14537, 995, 333 };
	public int thingsnottodrop2[] = { bronzeArrowID, ironArrowID, steelArrowID,
			mithrilArrowID, addyArrowID, runeArrowID, feathersID, 14537, 995,
			333 };
	...
	private State getState() {
		...
		if (!takeBones1 && !takeBones2 && inventoryContainsOneOf(bonesID)) {
			status = "Dropping junk";
			return State.DROPBONES;
		}
		...
		try {
			...
			case TOLOCATION:
				if (location.equals("Lumbridge East(near cowfield)")) {
					...
					if (3021 <= locX && locX <= 3025 && 3296 >= locY
							&& locY >= 3291) {
						walkTileOnScreen(new RSTile(3022, 3293));
					...
					} else if (3026 <= locX && locX <= 3029 && 3296 >= locY
							&& locY >= 3290) {
						walkTileOnScreen(new RSTile(3026, 3292));
						if (waitToMove(random(1000, 1500))) {
							while (getMyPlayer().isMoving()) {
								wait(random(20, 30));
							}
			...
			case ATTACK:
				...
				if (!pointOnScreen(chicken.getScreenLocation())) {
					walkTileOnScreen(randomizeTile(chickentile, 1, 1));
					if (waitToMove(random(1000, 1500))) {
						while (getMyPlayer().isMoving()) {
							wait(random(20, 30));
						}
			...
			case STOPSCRIPT:
				log("Stopping script because they are no more arrows");
				logout();
				stopScript();
				break;
			...
			case DROP:
				dropAllExcept(true, thingsnottodrop);
				break;

			case DROPBONES:
				dropAllExcept(true, thingsnottodrop2);
				break;
	...
	public boolean onStart(Map<String, String> args) {
		...
		if (location.equals("Champions guild")) {
			log("It is advised to start script when ");
			log("you are already in Champions Guild!");
		}
		if (location.equals("South of falador")) {
			log("It is advised to start script when you are ");
			log("already in South of falador chicken coop!");
		}
