...
@ScriptManifest(authors = { "Tekk & B-Warrior" }, category = "Combat", name = "BTekksCrabs", version = 2.7, description = "<i><b>BTekksCrabs v2.7</b> By Tekk & B-Warrior</i><br /><b>Start at the eastern crabs. </b>")
public class BTekksCrabs extends Script implements PaintListener {
	...
	RSTile[] BankToCrabs = new RSTile[] { new RSTile(2727, 3484),
			...
			new RSTile(2704, 3636), new RSTile(2707, 3655),
			new RSTile(2706, 3662), new RSTile(2705, 3673),
			new RSTile(2706, 3679) };
		...
		if (gui.chbUseSpec.isSelected()) {
			...
			if (gui.chbSwitchWeapon.isSelected()) {
				...
				if (!gui.tfMainWeapon.getText().equals("")) {
					try {
						MainWeapon = Integer.parseInt(gui.tfMainWeapon
								.getText());
					} catch (final NumberFormatException e) {
						log("Nummers only");
						stopScript();
				...
				if (!gui.tfSpecWeapon.getText().equals("")) {
					try {
						SpecWeapon = Integer.parseInt(gui.tfSpecWeapon
								.getText());
					} catch (final NumberFormatException e) {
						log("Nummers only");
						stopScript();
	...
	private boolean needToPickUpItems() {
		final RSItemTile arrowToPickUp = getGroundItemByID(3, pickupArrowID);
		...
		if (pickingupcharms) {
			final RSItemTile charmToPickUp = getGroundItemByID(3, charms);
			if (getMyPlayer().getLocation().getY() >= 3710) {
		...
		if (pickingupseaweed) {
			final RSItemTile seaweedToPickUp = getGroundItemByID(3, seaweed);
			if (getMyPlayer().getLocation().getY() >= 3710) {
	...
	public void CheckForSpec() {
		if (getSpec() >= precentSpecial && !clickedSpec() && usingSpec) {
			if (!SwitchWeapon) {
				openTab(TAB_ATTACK);
	...
	public void openBankScreen() {
		...
		final RSTile BankBoothTile = bankBooth.getLocation();
	...
	public void save(final Map<String, String> args) {
		try {
			final FileWriter fstream = new FileWriter("BTekksCrabs.ini", false);
	...
	public void withdrawnFood() {
		...
		final RSInterfaceComponent bankFood = bank.getItemByID(bankFoodID);
		final RSInterfaceComponent bankCammyID = bank.getItemByID(cammytab);
		if (bankFood == null) {
	...
	private void eatFood() {
		final int currentHP = skills.getCurrentSkillLevel(STAT_HITPOINTS);
		if (currentHP <= eatAtRandomHP) {
			if (getInventoryCount(foodID) >= 1) {
				for (final int i : foodID) {
	...
	private void walkNorth() {
		status = 5;
		final RSTile[] randomSTNPath = randomizePath(southToNorth, 2, 2);
		for (final RSTile aRandomSTNPath : randomSTNPath) {
	...
	private void walkToBank() {
		status = 6;
		final RSTile[] randomTTBPath = randomizePath(TeleSpotToBank, 1, 1);
		if (getEnergy() > random(maxEnergy, maxEnergy) && !isRunning()) {
			setRun(true);
		}
		for (final RSTile aRandomTTBPath : randomTTBPath) {
	...
	private void walkToCrabs() {
		status = 8;
		final RSTile[] randomBTCPath = randomizePath(BankToCrabs, 1, 1);
		if (getEnergy() > random(maxEnergy, maxEnergy) && !isRunning()) {
			setRun(true);
		}
		for (final RSTile aRandomBTCPath : randomBTCPath) {
	...
	private void walkSouth() {
		status = 4;
		final RSTile[] randomNTSPath = randomizePath(northToSouth, 2, 2);
		for (final RSTile aRandomNTSPath : randomNTSPath) {
	...
	private void findAndAttackCrabs() {
		status = 2;
		final RSNPC newCrab = getNearestFreeNPCByID(1266, 1268);
		final RSTile[] randomWTCPath = randomizePath(westTocenter, 2, 2);
		if (!justHaveBeenFighting) {
			if (getNearestNPCByID(1265, 1267) != null
				...
				if (wantToAttackCrabs) {
					...
					final int rndAtt = random(1, 3);
					final int rndNum = random(1, 4);
					if (rndNum == rndAtt) {
						clickRSNPC(getNearestNPCByID(1265, 1267), "Attack");
					} else {
		...
		if (newCrab != null && newCrab.getLocation().getX() > 2691
				...
				&& !justwokeacrab()) {
			final RSTile crabLocation = newCrab.getLocation();
			if (tileOnScreen(crabLocation)) {
	...
	public class gui extends javax.swing.JFrame {
		...
		private void initComponents() {
			...
			chbPowerTraining
					.setText("Power training? (only use this if you dont want pickup items)");
			...
			chbAllTabs
					.setText("Have you got your tele tabs with you ?(Camelot Teletabs)");

			jLabel17
					.setText("Enter food ID (The one that is withdrawn from bank):");

			jLabel18
					.setText("In which slot can the script find your food at ?(First slot = 1) :");

			chbOwnEnergy.setText("Use built-in random run energy");

			jLabel19
					.setText("This means if youre run energy is gone when may the script activate it again");
			...
			layout.setHorizontalGroup(layout.createParallelGroup(
					...
					jTabbedPane2, javax.swing.GroupLayout.DEFAULT_SIZE, 468,
					Short.MAX_VALUE));
			layout.setVerticalGroup(layout.createParallelGroup(
					javax.swing.GroupLayout.Alignment.LEADING).addComponent(
					jTabbedPane2, javax.swing.GroupLayout.DEFAULT_SIZE, 428,
					Short.MAX_VALUE));

			pack();
		}// </editor-fold>

		private void chbRandomEatingActionPerformed(
				final java.awt.event.ActionEvent evt) {
			// TODO add your handling code here:
		}

		private void btStartActionPerformed(final java.awt.event.ActionEvent evt) {
			startActionPerformed(evt);
		}

		private void rbDefenceActionPerformed(
				final java.awt.event.ActionEvent evt) {
			// TODO add your handling code here:
		}

		public void startActionPerformed(final java.awt.event.ActionEvent evt) {
	...
	public void onRepaint(final Graphics render) {
		...
		final Color background = new Color(0, 0, 0, 115);

		render.setColor(background);
		render.fill3DRect(10, 10, 175, 115, true);
		render.setColor(Color.white);
		render.drawString("BTekksCrabs v2.7", 15, 25);
